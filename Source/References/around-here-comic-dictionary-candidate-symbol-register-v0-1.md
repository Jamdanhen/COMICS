# Around Here Comic Dictionary - Candidate Symbol Register v0.1

Status: working symbol register / partially promoted

Source:

`C:\comics\Source\Uploads\source-note-around-here-comic-dictionary-register-2026-06-14.md`

Core symbol reference:

`C:\comics\Source\Canon\around-here-core-symbol.md`

## Purpose

This register is a working dictionary of symbols that may appear in the comic language of Around Here.

It mixes approved core symbols, promoted symbols, held candidates, infrastructure symbols, and high-risk ideas that should only be observed for now.

Do not treat every entry in this register as approved canon. Check each entry's status.

## Tier 1 - Foundational Symbols

### Around Here

Category:

Jurisdiction

Meaning:

- Origin point of mythology.
- Place where observations become symbols.

Status:

Core Symbol

### The Noodle

Category:

Unresolved Significance

Meaning:

- Known relevance.
- Unknown importance.

Question:

> "Why is this here?"

Status:

Core Symbol

### The Rubber Chicken

Category:

Absurdity

Meaning:

- Humor.
- Perspective.
- Community.
- Acceptance of the ridiculous.

Status:

Core Symbol

### The Rook

Category:

Structure

Meaning:

- Framework.
- Order.
- Civilized construction.

Related concepts:

- Repository
- Organization
- Architecture

Status:

Core Symbol

### Idea Spud / Living Information Blob

Category:

Early Formation / Living Information

Meaning:

- Idea or information before it outgrows containment.
- Early living information form.
- Seed form of a larger information blob.

Status:

Approved Symbol

Approval basis:

Approved through repeated use in Comics 1 through 3.

## Tier 2 - Strong Secondary Symbols

### The Crate

Category:

Classified Information

Meaning:

Unknown contents.

Question:

> "What is inside?"

Status:

Strong Candidate

### The Mine

Category:

Discovery / Excavation

Meaning:

- Place where digging and excavation occur.
- Rare ores become ideas or meaning.
- Active discovery rather than only deferred danger.

Question:

> "What happens when somebody hits it?"

Status:

Strong Candidate

### The Chronicle

Category:

Institutional Memory

Meaning:

- Recorded events.
- Historical continuity.

Function:

What happened.

Status:

Hold

### The Monocle

Category:

Interpretation

Meaning:

- Opinion.
- Analysis.
- Perspective.

Function:

What it means.

Status:

Hold

### The Gazelle

Category:

Observation / Reporting Character

Meaning:

- Field reporting.
- Immediate awareness.

Function:

What is happening now.

Status:

Authorized Character

Character authority:

`C:\COMICS\Source\Characters\character-gazelle-reporter.md`

Approved visual reference:

`C:\COMICS\Images\Generated\candidate-character-gazelle-reporter-2026-07-05-v3.png`

## Tier 3 - Emerging Symbols

### Zodak's Helmet

Category:

Unresolved Classification

Meaning:

Known contradiction.

Definition:

A question intentionally left open until a better framework appears.

Status:

Approved Symbol

Approval basis:

Approved by Taskmaster and included in the approved `Around Here's Most Wanted` comic as the `Master of Evil` unresolved-classification symbol.

### The Code Block

Category:

Unread Signal

Meaning:

- Potential information.
- Unknown translation.

Origin:

Unexpected interface artifact.

Status:

Emerging

### The Idea Spud

Category:

Early Formation

Meaning:

Concept before structure.

Definition:

Something becoming.

Status:

Approved Symbol

### The Blob

Category:

Information Growth

Meaning:

Ideas exceeding containment.

Definition:

Information mutating faster than the container can accommodate.

Status:

Approved Symbol

Approval basis:

Approved as part of the Idea Spud / Living Information Blob language and already used in early approved comics.

## Tier 4 - Infrastructure Symbols

### The Committee

Category:

Governance

Meaning:

- Rules.
- Oversight.
- Procedure.

Status:

Foundational Infrastructure Symbol

Approval basis:

Committee notice language appears in approved Symbol Key Volume 1.

### The Biological Committee

Category:

Physical Reality

Meaning:

- Sleep.
- Food.
- Hydration.
- Limitations.

Known weakness:

Interestingness.

Status:

Recurring / not separately differentiated yet

Open question:

How to visually distinguish a Biological Committee notice from a Committee for Comedic Standards notice without making the notice language too wordy.

### Shift Clock / Mine Time Clock

Category:

Externally Imposed Work Interval

Meaning:

- The work is not finished.
- The authorized interval has ended.
- Technological or platform limits interrupt active discovery.

Status:

Approved Symbol

Approval basis:

Taskmaster approval on 2026-07-05.

Draft image:

`C:\COMICS\Images\Generated\candidate-symbol-shift-clock-mine-time-clock-2026-07-04.png`

### The Approved Shelf

Category:

Canon

Meaning:

Decision finalized.

Status:

Infrastructure

### The Repository

Category:

Preservation

Meaning:

Information survives.

Status:

Infrastructure

## Tier 5 - High-Risk Symbols

These ideas are interesting but not stable.

### Play-Doh

Placeholder name.

Category:

Interoperability

Meaning:

Connector between systems.

Risk:

May become too broad.

Status:

Observe

### The Cyborg

Category:

Integration

Meaning:

Human + System.

Risk:

Premature definition.

Status:

Observe

### Spiral Noodle

Category:

Recursion

Meaning:

- Self-reference.
- Escalating complexity.

Risk:

Still poorly defined.

Status:

Observe

## Promotion Rule

A symbol is promoted when:

1. It survives.
2. It recurs.
3. It acquires meaning.
4. It solves explanatory problems.
5. It remains useful after the joke ends.

## Around Here Test

Question:

> "If this disappeared, would the mythology lose something?"

If yes:

The symbol remains.

If no:

The symbol was probably only a joke.

## First Dictionary Page Shortlist

Taskmaster's current priority list for the first actual comic dictionary page:

- Around Here
- The Noodle
- The Rubber Chicken
- The Rook
- The Crate
- Zodak's Helmet

Reason:

These six feel like they carry the most conceptual weight right now without requiring too much explanation.

The remaining symbols currently feel more like residents of Around Here than foundational landmarks.

## 2026-07-05 Promotion Notes

Taskmaster approved or clarified the following:

- Shift Clock / Mine Time Clock is approved.
- Zodak's Helmet is approved.
- Idea Spud / Living Information Blob is approved.
- Chief Inspector is an approved character, not merely a symbol.
- Around Here Chronicle is on hold.
- Monocle is on hold.
- Gazelle Reporter is authorized as a character.
- Committee notices are already part of foundational visual language through Symbol Key Volume 1.
- All symbols appearing in the approved `Around Here's Most Wanted` comic are approved by inclusion.
- Symbol Key Volume 2A is not approved as a sheet; most entries lack enough context, except Noodle Shop remains interesting, Mythic BEAM needs a draft, and Idea Spud is already approved.
